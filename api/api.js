...
export function api(request, service, data, token) {
    return request({
        url: config.public.baseURL + service,
        // url: "https://yadapi.kerloper.com/" + service,
        method: 'post',
        config: {
            proxyHeaders: false,
            credentials: false,
...
export function fetchData(request, opts) {
    const config = useRuntimeConfig()
