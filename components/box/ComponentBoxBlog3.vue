...
<template>
  <div class="relative pointer cursor-pointer px-1">
    <nuxt-link :to="'/blog/'+item.slug">
      <div class=" hover:shadow-xl shadow-md">
        <div class="relative group overflow-hidden rounded-tl-xl rounded-tr-xl ">
          <img :src="item.image.src" alt="swiper" class="object-cover w-full h-44 sm:w-full
            group-hover:scale-110 duration-500 transition   ease-in-out"
          />
        </div>
        <!--        description-->
        <div class="flex flex-col bg-white rounded-br-lg rounded-bl-lg px-4">
